...
interface Card { id: string; card_name: string; last_four: string; card_type: string; is_active: boolean; expires_year: string; holder_id: string; account_id: string; }
interface CreateCardForm { account_id: string; holder_id: string; card_name: string; card_type: "virtual" | "debit"; expires_month: string; expires_year: string; }
...
export default function CardsPage() {
  ...
  const [form, setForm] = useState<CreateCardForm>({ account_id: "", holder_id: "", card_name: "", card_type: "virtual", expires_month: "", expires_year: "" });
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API}/api/finance/cards`, {
        method: "POST",
        headers: getHeaders(),
        body: JSON.stringify({ ...form, expires_month: parseInt(form.expires_month), expires_year: parseInt(form.expires_year) }),
      });
      if (!res.ok) { const d = await res.json(); throw new Error(d.detail ?? "Failed"); }
      setShowCreate(false);
      setForm({ account_id: "", holder_id: "", card_name: "", card_type: "virtual", expires_month: "", expires_year: "" });
      load();
    } catch (e: any) {
          ...
          {cards.map((c, i) => (
            <motion.div key={c.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}>
              <div style={{
                ...
                boxShadow: `4px 4px 0 0 ${c.card_type === "debit" ? "rgba(151,25,44,0.3)" : "rgba(252,146,13,0.3)"}`,
